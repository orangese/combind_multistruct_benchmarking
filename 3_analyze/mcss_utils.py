...
class MCSS:
    ...
    def load_mcss(self, lset1, lset2, rmsd=False):
        for l1 in lset1:
            for l2 in lset2:
                if l1 > l2 and not (l1[:6] != 'CHEMBL' and l2[:6] == 'CHEMBL'):
                    l1,l2 = l2,l1
                if (l1,l2) not in self.all_pairs:
                    self.all_pairs[(l1,l2)] = PairMCSS(l1,l2,self.sp,self.root)
                    self.all_pairs[(l1,l2)].load_mcss_size()
                if rmsd: 
                    self.all_pairs[(l1,l2)].load_mcss_rmsd(self.num_poses)
...
class PairMCSS:
    def __init__(self, l1, l2, sp, root):
        self.l1 = l1
        self.l2 = l2
        self.sp = sp
        self.root = root
    ...
    def get_mcss_path(self,add_dir=False,add_st=False,ext='csv'):
        pth = '{}-{}-{}'.format(self.l1,self.l2,self.sp['mcss_type'])
        if add_st: pth = '{}-{}-{}'.format(pth,self.st,self.sp['docking'])
        if add_dir: pth = '{}/mcss/{}/{}-{}/{}'.format(self.root,self.sp['mcss'],self.l1,self.l2,pth)
        return '{}.{}'.format(pth,ext)

    def load_mcss_size(self):
        try:
            with open(self.get_mcss_path(add_dir=True,ext='size')) as f:
    ...
    def load_rmsds(self, num_poses):
        ...
        pth = get_mcss_path(add_dir=True,add_st=True)
        if os.path.exists(pth) and os.stat(pth).st_size == 0:
            print 'deleting empty file',fpath
        ...
        try:
            with open(pth) as f:
                for line_count, line in enumerate(f):
                    ...
                    elif p1+1 < min(num_poses[l1], 100) or p2+1 < min(num_poses[l2], 100) or rmsd == float(10000):
                        print line_count, 'hmmmm', fpath, p1, p2, num_poses[l1], num_poses[l2], rmsd
