from utils import StringFunction

STATS = {'default': {'version'         : 'default',
                     'ifp_version'     : 'default',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'paper': {'version'          : 'paper',
                     'ifp_version'     : 'default',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'paper',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'paper_rd1': {'version'      : 'paper',
                     'ifp_version'     : 'rd1',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'paper',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},

          'rd1':    {'version'         : 'rd1',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'pharm_max',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'rd1_min':{'version'         : 'rd1_min',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'pharm_min',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},

          'rd1_mmod':{'version'        : 'rd1_mmod',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'mmod_max',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'rd1_mmod_min':{'version'         : 'rd1_mmod_min',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'mmod_min',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},

          'rd1_phase':{'version'       : 'rd1_phase',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'qsar_max',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'rd1_phase_min':{'version'   : 'rd1_phase_min',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'qsar_min',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},


          'ifd':    {'version'         : 'ifd',
                     'ifp_version'     : 'rd1',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'ifd',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'ifd2':    {'version'        : 'ifd2',
                     'ifp_version'     : 'rd1',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'ifd2',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'rd1_all': {'version'        : 'rd1_all',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'pharm_max',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4_all',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'rd1_all_equal': {'version'  : 'rd1_all_equal',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'pharm_max',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4_all',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100,
                     'ligand_pairs_equal': True},

          'rd2_all': {'version'        : 'rd2_all',
                     'ifp_version'     : 'rd2',
                     'shape_version'   : 'pharm_max',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4_all',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'rd1_xp_all': {'version'     : 'rd1_xp_all',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'pharm_max',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'XP_all',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'rd1_core':{'version'       : 'rd1_core',
                     'ifp_version'     : 'rd1',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'core',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
          'rd2':    {'version'         : 'rd2',
                     'ifp_version'     : 'rd2',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'confgen_es4',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},
        'shape': {'version'        : 'shape',
                     'ifp_version'     : 'rd1',
                     'shape_version'   : 'pharm_max',
                     'mcss_version'    : 'mcss16',
                     'mcss_func'       : StringFunction('min'),
                     'mcss_rel_min'    : 0.5,
                     'mcss_abs_min'    : 10,
                     'mcss_domain'     : (0, 6),
                     'docking_version' : 'shape',
                     'pdb_order'       : 'First',
                     'native_thresh'   : 2.0,
                     'n_ligs'          : 20,
                     'max_poses'       : 100,
                     'stats_sd'        : 0.03,
                     'stats_points'    : 100},

                     }

IFP = {'default': {'version'           : 'default',
                   'hbond_dist_opt'    : 2.5,
                   'hbond_dist_cut'    : 3.0,
                   'hbond_angle_opt'   : 60.0,
                   'hbond_angle_cut'   : 90.0,
                   'sb_dist_opt'       : 4.0,
                   'sb_dist_cut'       : 5.0,
                   'contact_scale_opt' : 1.25,
                   'contact_scale_cut' : 1.75},
        'rd1':    {'version'           : 'rd1',
                   'level'             : 'residue',
                   'hbond_dist_opt'    : 3.5,
                   'hbond_dist_cut'    : 4.0,
                   'hbond_angle_opt'   : 60.0,
                   'hbond_angle_cut'   : 90.0,
                   'sb_dist_opt'       : 4.0,
                   'sb_dist_cut'       : 5.0,
                   'contact_scale_opt' : 1.25,
                   'contact_scale_cut' : 1.75},
        'rd2':    {'version'           : 'rd2',
                   'level'             : 'atom',
                   'hbond_dist_opt'    : 3.5,
                   'hbond_dist_cut'    : 4.0,
                   'hbond_angle_opt'   : 60.0,
                   'hbond_angle_cut'   : 90.0,
                   'sb_dist_opt'       : 4.0,
                   'sb_dist_cut'       : 5.0,
                   'contact_scale_opt' : 1.25,
                   'contact_scale_cut' : 1.75}
      }

FEATURE_DEFS = {
    'shape'          : [],
    'mcss'           : [],
    'sb'             : [1],
    'hbond'          : [2,3],
    'hbond_donor'    : [2],
    'hbond_acceptor' : [3],
    'contact'        : [11]
}

PATHS = {'ROOT': '{DATA}/{protein}',

         'LIGANDS_ROOT': '{ROOT}/ligands',
         'LIGANDS': '{LIGANDS_ROOT}/{ligand}/{ligand}.mae',

         'GRID_ROOT': '{ROOT}/docking/grids',
         'GRID': '{GRID_ROOT}/{struct}/{struct}.zip',
         'DOCK': '{ROOT}/docking/{docking_version}/{ligand}-to-{struct}',
         'DOCK_PV': '{DOCK}/{ligand}-to-{struct}_pv.maegz',
         'DOCK_REPT': '{DOCK}/{ligand}-to-{struct}.rept',
         'DOCK_RMSD': '{DOCK}/rmsd.csv',
         'IFP_ROOT':  '{ROOT}/ifp/{ifp_version}',
         'IFP':  '{IFP_ROOT}/{ligand}-to-{struct}-{docking_version}.fp',
         'MCSS': '{ROOT}/mcss/{mcss_version}',
         'SHAPE': '{ROOT}/shape/{shape_version}',
         }
